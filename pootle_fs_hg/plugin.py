...
class HgPlugin(Plugin):
    ...
    @property
    def repo(self):
        return hglib.open(self.project.local_fs_path)
        ...
        try:
            with tmp_branch(self) as branch:
                if response.made_changes:
                    logger.info(
                        "Committing/pushing hg repository(%s): %s"
                        % (self.project.code, self.fs_url))
                    add_paths = [
                        os.path.join(
